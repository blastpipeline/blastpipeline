...
PIPELINE_FULL_SIZE = 1000000 #pipeline is too full, go for another
class Conn():
    def __init__(self):
        ...
        self.freetime = 0 #When is this connection "free" for further activity? For managing Trans Ready
...
##            if started_server == 0:
        ...
        if ename == "Init Conn":
            ...
            if (HAS_OPTDATA == 0):
                ...
                timer += 0.05 #small parameter for end node round-trip time
            ...
            #this part is never called. we skip straight to conn done.
        ...
        if ename == "TLS Done":
            ...
            self.conns[eind].readytime = self.cur_t
            #For better pipelining, CT is walked when TLS is done, not when conns are done
            #So we don't trick transactions with fake pipelines
    ...
    def TryDispatchTrans(self, tr):
        ...
        log("Ent {} trying to dispatch trans {}, {}/{}/{} connections, queue {}".format(self.ents.index(tr.ent),
                                                                                        self.trans.index(tr),
        ...
        #(1) If there are any idles, choose one
        #This is reverse of the usual order, for better performance
        conns = tr.ent.get_conns_by_state(1)
        if conns != []:
            conn = conns[0]
            log("Idle conn {} (ent {}) dispatches trans {}".format(self.conns.index(conns[0]),
...
##                    break
        ...
        #(2) If there are any non-full pipelines, get the shortest such pipeline, then send this transaction
        ...
        #qualify conns
        for c in conns:
            if len(c.trans) * c.trans[0].res.hasWritten < PIPELINE_FULL_SIZE:
                qconns.append(c)
        ...
        if len(conns) != 0:
            ...
            if len(s_conn.trans) < tr.ent.pipesize:
                log("Conn {} (pipelined with {}) dispatches trans {}".format(self.conns.index(s_conn),
                                                                             len(s_conn.trans),
                                                                             self.trans.index(tr)))
...
options = [[1, 0, 10, 10, ".simbdelay4"],
           [1, 0, 4, 20, ".simbdelay7"],
           [1, 0, 4, 20, ".simbdelay8", 1, 0, 0],
           [1, 0, 4, 20, ".simbdelay9", 1, 1, 0],
           [1, 0, 4, 20, ".simbdelay10", 1, 1, 1]]
...
for opts in options:
    ...
    if len(opts) >= 8:
        HAS_OPTDATA = opts[5]
        HAS_SHAVEREDIRECT = opts[6]
        HAS_ZEROTLS = opts[7]

    print HAS_OPTDATA

    if HAS_SHAVEREDIRECT != 0:
    ...
    for fname in fnames:
        ...
        sorted_rlist = sorted(rlist, key = lambda r:r.lastwritetime)
        ttimes.append(sorted_rlist[-1].lastwritetime - sorted_rlist[0].createdtime)
